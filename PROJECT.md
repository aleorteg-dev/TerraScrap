...
| 2026-06-11 | B5/F1 import jobs | *(entrada retroactiva, IT-DOC-1/G03 — commit `8181061`)* Nuevos endpoints `POST /api/world-imports` (202 → `job_id`, parseo en hilo de fondo con progreso) y `GET /api/world-imports/{job_id}` (`ImportJobStatusDto`: `status queued/processing/done/error`, `pct`, `world_id`+`metadata` al terminar, `error_code`/`error_message` en fallo). F1 `uploadWorld` migra a esta vía con polling. Presentes en `openapi.json`/snapshot desde entonces; especificación completa en `api-contract.md` desde IT-DOC-1. | iter-08-import-jobs |
...
| 2026-07-17 | B5 api-rest | Cachés de lectura (cierra P02, P06): `GET /tiles` cachea el chunk codificado — LRU `(world_id, cx, cy, size, encoding) → (w, h, payload)` de 512 entradas; la 2ª petición sirve el payload idéntico sin re-encodear. `GET /tile` usa índices `(x,y) → id` de chests/signs/tile_entities (`_WorldPosIndex`, construido on-demand y cacheado por mundo, LRU de 4) en vez de scans lineales; ante posiciones duplicadas gana la primera (semántica del `break` anterior, testeada). Patrón unificado en la clase `_LruCache[K, V]` (la caché de surface de IT-13 migra a ella); DELETE invalida las tres cachés. Sin cambio de contrato. | IT-14 |
| 2026-07-17 | B1 wld-parser | Contrato v2.7 (cierra E11, E19, M08, D06, G08): decodificación de strings garantizada sin excepción — cadena `utf-8` → `cp1252` → `latin-1` (0x81/0x8D/0x8F/0x90/0x9D no existen en cp1252 y reventaban el parse como `corrupt`); **breaking menor**: `TileGrid` deja de ser hashable (`hash` lanzaría sobre ~20M tiles → `TypeError` explícito) y sin `__eq__` estructural (tests con `assert_grids_equal`); rango soportado con fuente única en `_constants.py` (`MIN/MAX_SUPPORTED_VERSION`, consumido por parser y default de `UnsupportedWorldVersionError`); `except` con subclase redundante simplificado; `_classify_size` tipada con `Literal` (fuera el `type: ignore`). | IT-15 |
| 2026-07-17 | B2 world-repository | Contrato v2.0 **breaking** (cierra M02, P10): eliminados `touch()` (el `get()` ya renueva TTL; sin caller de producción) y `delete()` no estricto (iter-032 migró el router a `delete_strict`); `create_in_memory_repository(..., max_worlds: int | None = None)` — al almacenar el mundo N+1 primero purga expirados y luego expulsa el menos recientemente accedido (LRU); `get()` del expulsado → `WorldNotFoundError`; `max_worlds < 1` → `ValueError`; default `None` = sin cap. Follow-up B6 anotado: exponer `TWI_MAX_WORLDS`. | IT-16 |
| 2026-07-17 | B3 item-catalog | Limpieza de excepts muertos y tipado sin `Any` (cierra M05, M06, M07, G07): `_parse_id` sin `OverflowError` (imposible en `int(str)`); eliminado el `try/except WikiUnavailableError: raise` no-op; `except httpx.TransportError` único (TimeoutException es subclase); caché leída como `_CachePayload`/`_CacheItem` y scraper tipado con `_ScrapedItem`/`_ItemPatch` (TypedDict). **Decisión**: `item_id` duplicado en la fuente se dedupe conservando la primera entrada con WARNING (antes `get` devolvía el último y `search` listaba ambos). Sin cambio de contrato. | IT-17 |
| 2026-07-17 | infra frontend | Configs unificadas (cierra M17, M18/D07, G06): eliminado el script npm duplicado `generate:api-types` (queda `gen:api`); `vitest.config.ts` borrado y su bloque `test` (el vigente: `.spec.` + js/jsx) movido a `vite.config.ts` — única config de Vite y Vitest; bajo Vitest el plugin react se omite (`process.env.VITEST`; plugin-react@6 requiere vite@8 y vitest@3 embebe vite@7, decisión iter-007), JSX de tests vía esbuild automático; `tsconfig.node.json` typechequea ahora `vite.config.ts` (el typecheck nuevo cazó de inmediato el conflicto de tipos vite7/vite8, resuelto con cast documentado). Verificado: mismos 269 tests, `dev-proxy.test.ts` verde, build verde. | IT-18 |
| 2026-07-17 | B3 item-catalog | Enrich del scraper paralelo (cierra P09): `_enrich_items` corre las páginas por ítem con `asyncio.Semaphore(6)` + `gather` (antes ~6 146 páginas secuenciales); backoff por petición intacto en `_get_with_retry`; semántica preservada (404 → skip con warning; primer 5xx aborta con `WikiUnavailableError` sin escribir caché, vía `return_exceptions=True` + re-raise). Solo CLI (`refresh.py`), sin impacto en runtime del servidor. | IT-OPT-6 |
| 2026-07-17 | B1 wld-parser | Contrato v2.8 (IT-OPT-2, primer eslabón de la cadena E14): `Tile` expone propiedades de wiring de solo lectura — `wire_red/wire_blue/wire_green` (flags2 bits 1–3), `actuator` (flags3 bit 1), `wire_yellow` (flags3 bit 5) — derivadas del bitmask `flags` ya almacenado (sin coste de memoria por tile). Builder de fixtures admite `wires_at` para inyectar headers 2/3. Siguiente eslabón: B5 emite estos bits en el encoding v2 (IT-OPT-3). | IT-OPT-2 |
| 2026-07-17 | B5 api-rest | Encoding v2 emite wiring (IT-OPT-3, cadena E14 2/4): bits 1–5 del byte `flags` de cada run llevan actuator + wires rojo/azul/verde/amarillo reales (antes reservados a 0); `_tile_eq_v2` separa runs que difieren solo en wiring. No breaking (los decoders ya preservaban el byte); `api-contract.md` §5.2 actualizado; sin cambio de schema OpenAPI. | IT-OPT-3 |
| 2026-07-17 | F3 world-canvas | Wires pintados con bits reales (IT-OPT-4, cadena E14 3/4): `WIRE_COLORS` + `getWireColor(flags)` en `tileColors.ts` (bits 2–5 del byte flags v2, un color por cable, prioridad rojo > azul > verde > amarillo; actuator no pinta); el paso 4 del renderer v2 los usa con `showWires`. Sin cambio de contrato público. | IT-OPT-4 |
| 2026-07-17 | F6 app-shell | Toggle "Cables" rehabilitado (IT-OPT-5, cadena E14 4/4 — **cierra la solución real de E14**): botón sin `disabled`/`title`; togglear muestra el cableado real pintado por F3 (re-rasterizado local sin red). Default OFF conservado (capa opcional, como TerraMap). | IT-OPT-5 |
| 2026-07-17 | B1 wld-parser | Índices de posición sin 2ª pasada (IT-OPT-1, cierra P04): `_read_tiles` construye los índices por run RLE mientras decodifica (`array.extend(range(...))` a velocidad C) y crea el grid con la factoría interna `TileGrid._from_parsed`; la re-visita eager de ~20 M tiles desaparece del parseo. `TileGrid(columns)` público intacto (eager como siempre); test de equivalencia run-built == rebuild eager. **Decisión**: sin numpy no se migra a columnar — la grid ya comparte el objeto `Tile` por run (la columnar por campos duplicaría memoria) y una pool-indexed exigiría otra pasada; se reevalúa si numpy entra al stack. Presupuestos RNF verificados (`pytest -m perf`: parse <10 s, search <500 ms). | IT-OPT-1 |
| 2026-07-17 | docs / gobernanza | Solo documentación (cierra G01, G02, G03, G04, G09, N04, D08): CLAUDE.md — rango del parser corregido a **v230–v319** (apuntando a `_constants.py` como fuente única), TTL de sesión configurable con **`TWI_WORLD_TTL_SECONDS`** (no `TWI_MAX_UPLOAD_MB`), filas de `/api/world-imports` en la tabla de endpoints y comando de cobertura `pytest --cov=twi` (IT-00C); `api-contract.md` — especificación completa de `POST/GET /api/world-imports` (202 + polling + códigos + retención IT-01, sin cambio de schema OpenAPI) y fuente única del límite de subida documentada en §3 (`.env` → Settings → router/middleware → `UploadWorld`; nginx 250m = headroom deliberado); `.gitignore` — cabecera React 18 → React 19; changelog — entrada retroactiva de los import jobs (commit `8181061`) y nota de corrección de métricas en §14 (Dockerfile alpine, bundle ~94 kB gzip). | IT-DOC-1 |
...
> **Corrección (IT-DOC-1, G09/N04)**: dos métricas de entradas antiguas de este
> changelog quedaron desfasadas respecto al repo real: `backend.Dockerfile` usa
> base **`python:3.12-alpine`** (no `-slim` como decía la entrada de iter-020) y
> el bundle del frontend medido en 2026-07 es **~287 kB / ~94 kB gzip** (no los
> 73.82 kB gzip de la entrada de cierre v0.2; el crecimiento viene de las
> paletas/nombres de tiles y los módulos F3–F6 añadidos desde entonces — sigue
> holgadamente bajo el target < 2 MB). Las entradas originales se conservan tal
> cual como registro histórico.
